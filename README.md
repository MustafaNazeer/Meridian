...
* **L1 today, L8 with the extended snapshot follow-up.** The Ladder displays the live top of book on the row closest to the spread; the other seven rows on each side are placeholders pending the extended snapshot payload (L8 depth, recent trades, engine latency histogram). The section aux labels make this explicit.
...
The build is sequenced into shippable milestones (foundations, single-symbol matching, multi-instrument and cancel, property tests, concurrency, the bench push, ITCH replay, the remaining order types, the WebSocket server, the frontend, the deploy, the public README rewrite). All twelve are landed; this README itself is the closing artifact. From here, the open work is the v1.1 follow-ups: extending the WebSocket snapshot payload to carry L8 depth, recent trades, and engine latency percentiles, so the four panels currently rendering placeholder anatomy can show real data.
