...
This file describes each top-level component in the Meridian dashboard in plain prose, one section per component. It is the layout contract for the React port. The visual ground truth is `/home/mustafa/src/Meridian/docs/design/canonical.html`; the token contract is `/home/mustafa/src/Meridian/docs/design/tokens.md`. Wireframes here describe layout, anatomy, responsive behavior, and micro-interactions; they do not duplicate token values.

Every visual region in the canonical HTML is annotated with a `data-component` attribute (or will be once the React port is built) and each meaningful sub element is annotated with `data-element`. These attributes are the search keys for component code, tests, accessibility audits, and any future design change. Always preserve them in the React port; they double as test selectors and as Storybook labels.
...
* The shell mounts a Zustand-backed connection state machine and a single WebSocket client (`frontend/src/ws/client.ts`); the four states are `connecting`, `live`, `stalled`, `disconnected`.
...
* Micro-interactions:
  * The stat values flash a 600 ms gold tint when their underlying datum changes (see tokens.md `duration-tape-flash`). The flash is a single-shot `animation`, not a `transition`, so rapid updates do not compound. Verify the flash against the canonical before enabling in production.
...
* Micro-interactions:
  * On row hover, the row gains a faint `surface` underlay; the trigger should be CSS-only (`tr:hover td`). No tooltip on first cut; the FIFO tooltip is deferred to a later polish pass and should land only if it can be ensured not to conflict with screen reader announcements.
...
* Micro-interactions:
  ...
  * Hover behavior: deferred to a later polish pass. If implemented, a vertical hover guide draws a 1 px `ink-soft` line at the cursor x and renders a small floating tooltip in `surface-overlay` with cumulative depth at that price. Verify the affordance has a keyboard-accessible alternative before adding.
...
Two of the four connection states (`connecting`, `disconnected`) require visible affordances beyond the header dot. Their layout integrations are:
...
These behaviors are wireframed here so they do not have to be invented during the React port. If a state's surface looks wrong against the canonical when implemented, fix the wireframe here first, then the canonical or tokens.
...
* The frontend can build each component without needing to invent layout decisions; ambiguities resolve here, in writing, before they resolve in code.
* The four connection states each have a documented surface here.
