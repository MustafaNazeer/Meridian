...
The repository is public on GitHub at `MustafaNazeer/Meridian`. **No secret value is ever committed to the repo, in any branch, ever.** All secrets live either in GitHub Actions repository secrets, in Fly.io app secrets, or in the user's password manager. `gitleaks` runs in CI as a required check and will fail any PR that introduces a credential pattern. The pre-commit hook described in `CLAUDE.md` provides a second line of defense.

The Fly.io account and the Cloudflare account themselves are protected by the user's password and a 2FA device; those credentials are not stored in this repo and are out of scope for this document.
...
* **Who can rotate:** the user (Mustafa Nazeer). The `flyctl auth tokens` CLI manages tokens.
* **Rotation procedure:**
...
* **Rotation cadence:** rotate once before the demo goes live publicly (Phase 10). After that, rotate on demand whenever the token may have leaked, and otherwise at least once every 12 months.
...
* **Where it is consumed:** if Phase 10 wires up a `wrangler` deploy step (rather than relying on the Cloudflare Pages GitHub integration), the workflow exports `CLOUDFLARE_API_TOKEN` into the step's environment. It is not exposed to PR runs from forks. If Phase 10 instead uses the Pages GitHub integration, this token is unused at the workflow level and the integration handles auth via Cloudflare's GitHub App; the token is still kept in the repository secret store as a fallback.
...
* **Who can rotate:** the user. Cloudflare's dashboard at "My Profile -> API Tokens" is the management surface.
* **Rotation procedure:**
...
* **Rotation cadence:** any app secret introduced after Phase 0 must be rotated before the demo goes live publicly, and then on demand or at least every 12 months.
...
* **The Fly.io account password and 2FA seed.** Lives in the user's password manager. Not in the repo, not in GitHub Actions, not in Fly app secrets. Out of scope for this document.
* **The Cloudflare account password and 2FA seed.** Same.
* **The user's GitHub credentials.** Same.
...
The following commands should be run by the Security Engineer or by CI before any deploy and any phase close:
