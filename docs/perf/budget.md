...
|---|---|---|---|---|
| `observability=false`, v1.1.0 (matching only, no depth cache) | 4.79 M evt/s | 149 ns | 618 ns | 1332 ns |
| `observability=true`,  v1.1.0 (matching + depth via `std::map` walk + trades + histogram) | 2.79 M evt/s | 288 ns | 903 ns | 2128 ns |
| `observability=false`, v1.1.1 (matching only, cache maintenance gated off) | 4.80 M evt/s | 151 ns | 615 ns | 1310 ns |
| `observability=true`,  v1.1.1 (matching + depth via cache + trades + histogram) | 3.39 M evt/s | 230 ns | 774 ns | 1593 ns |

v1.1.0 instrumented path cost roughly 40 percent of bench-mode throughput. v1.1.1 lands a per-side `Level*` cache on `Book` (top `kDepthLevels` entries) maintained by `Book::add` and `Book::remove_by_id` when the new `Book` observability flag is set, so `Book::publish_depth` reads through cached pointers instead of walking `std::map` on every event. The flag is threaded through `BookRegistry` so the bench can disable the cache maintenance alongside the existing `MatchingEngine` observability flag; the live demo keeps the default `true` so all four panels render real data. Remaining instrumented overhead drops from 142 ns/event to about 88 ns/event (about 38 percent reduction), and observability=true throughput recovers from 2.79 M evt/s to 3.39 M evt/s (about 21 percent gain). The bench-mode (`observability=false`) matching path is unchanged within run variance.

The bench binary (`apps/bench/main.cpp`) continues to build with `observability=false`. The headline throughput is the v1 matching path; `bench/baseline.json` continues to track the matching loop in isolation. Future work that wants to defend an instrumented-path budget should publish a second baseline with `observability=true` rather than blending the two.
...
| 2026-05-13 | v1.1.1 depth cache lands. `Book` maintains a per-side `std::array<Level*, kDepthLevels>` cache, updated by `Book::add` (insert with shift), `Book::remove_by_id`, and `Book::erase_empty_level` (shift up plus single-step refill from the price map). `Book::publish_depth` reads through the cache instead of walking the map on every event. Bench `observability=true` throughput improves from 2.79 M evt/s to 3.39 M evt/s on the desktop reference (about 21 percent), p50 latency from 288 ns to 230 ns. Cache maintenance is gated behind a new `Book` observability flag, plumbed through `BookRegistry`, so the bench (`observability=false`) skips it entirely and the matching path is unchanged within run variance. |
