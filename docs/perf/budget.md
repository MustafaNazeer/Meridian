...
| `observability=false`, v1.1.1 (matching + cache maintenance on add/remove) | 4.60 M evt/s | 162 ns | 646 ns | 1314 ns |
| `observability=true`,  v1.1.1 (matching + depth via cache + trades + histogram) | 3.39 M evt/s | 230 ns | 774 ns | 1593 ns |

v1.1.0 instrumented path cost roughly 40 percent of bench-mode throughput. v1.1.1 lands a per-side `Level*` cache on `Book` (top `kDepthLevels` entries) maintained by `Book::add` and `Book::remove_by_id`, so `Book::publish_depth` reads through cached pointers instead of walking `std::map` on every event. Remaining instrumented overhead drops from 142 ns/event to 78 ns/event (about 45 percent reduction), and observability=true throughput recovers from 2.79 M evt/s to 3.39 M evt/s (about 21 percent gain). The cache also adds about 4 percent overhead on the observability=false path (linear scan and shift on the add/remove hot path) which remains well above the 4.10 M evt/s CI baseline floor.
...
| 2026-05-13 | v1.1.1 depth cache lands. `Book` maintains a per-side `std::array<Level*, kDepthLevels>` cache, updated by `Book::add` (insert with shift), `Book::remove_by_id`, and `Book::erase_empty_level` (shift up plus single-step refill from the price map). `Book::publish_depth` reads through the cache instead of walking the map on every event. Bench `observability=true` throughput improves from 2.79 M evt/s to 3.39 M evt/s on the desktop reference (about 21 percent), p50 latency from 288 ns to 230 ns. `observability=false` regresses about 4 percent (from 4.79 to 4.60 M evt/s) due to the cache maintenance cost on the add/remove hot path; the result remains well above the CI baseline floor. |
